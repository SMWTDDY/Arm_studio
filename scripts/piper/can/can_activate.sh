...
###############################################################################
# 脚本名称: can_activate.sh
# 脚本功能: 激活并配置单路 CAN 模块 (支持自动重命名与 USB 地址绑定)
# 使用方法: 
#   sudo bash can_activate.sh [自定义名称] [比特率] [USB 硬件地址]
# 参数说明:
#   $1 - 自定义名称 (默认: can0)
#   $2 - 比特率 (默认: 1000000，即 1M)
#   $3 - USB 硬件地址 (可选，多路 CAN 时必填，如 1-2:1.0)
# 
# 常见用法示例:
#   1. 激活默认 can0: sudo bash can_activate.sh
#   2. 激活并命名为 can_master: sudo bash can_activate.sh can_master 1000000
#   3. 指定 USB 插口激活: sudo bash can_activate.sh can0 1000000 1-2:1.0
###############################################################################
...
if [ "${#DEFAULT_CAN_NAME}" -gt 15 ]; then
    echo "Error: CAN interface name '$DEFAULT_CAN_NAME' is too long (${#DEFAULT_CAN_NAME} chars)."
    echo "Linux network interface names must be 15 characters or fewer."
    echo "Suggested names: can_ml, can_sl, can_mr, can_sr."
    echo "-------------------ERROR-----------------------"
    exit 1
fi
...
if [ "$IS_LINK_UP" = "yes" ] && [ "$CURRENT_BITRATE" -eq "$DEFAULT_BITRATE" ]; then
    ...
    if [ "$INTERFACE_NAME" != "$DEFAULT_CAN_NAME" ]; then
        ...
        if ! sudo ip link set "$INTERFACE_NAME" name "$DEFAULT_CAN_NAME"; then
            echo "Error: failed to rename interface $INTERFACE_NAME to $DEFAULT_CAN_NAME."
            echo "-------------------ERROR-----------------------"
            exit 1
        fi
        if ! sudo ip link set "$DEFAULT_CAN_NAME" up; then
            echo "Error: failed to reactivate interface $DEFAULT_CAN_NAME."
            echo "-------------------ERROR-----------------------"
            exit 1
        fi
