...
# 如果用户显式传入 [CAN接口名称] [比特率] [USB硬件地址]，
# 直接按单路 CAN 模块配置，避免落入多 CAN 批量预设检查。
if [ "$#" -ge 3 ]; then
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    exec bash "$SCRIPT_DIR/can_activate.sh" "$1" "$2" "$3"
fi
...
if [ "$EXPECTED_CAN_COUNT" -ne 1 ]; then
    declare -A USB_PORTS 
    USB_PORTS["1-3:1.0"]="can_ml:1000000"
    USB_PORTS["1-1:1.0"]="can_sl:1000000"
    USB_PORTS["1-10:1.0"]="can_mr:1000000"
    USB_PORTS["1-9:1.0"]="can_sr:1000000"
