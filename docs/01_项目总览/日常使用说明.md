...
```bash
python inference/server.py --port 5555
```

当前仓库保留了 `inference/server.py` 和 `inference/client.py` 基础模块；历史文档中的
`scripts/run_inference.py` 与断掉的 `scripts/inference.sh` 已删除。local/remote 闭环推理
需要后续恢复 runner 或接入新的执行器。真实 Piper 遥操、采集、回放入口见 `scripts/piper/`。
