...
- **存储结构**：`HDF5Recorder` 已更新，不再使用嵌套的 `data/obs` 结构，而是直接对齐 `Api_test` 的 HDF5 键名：
  ...
  - `timestamp`: 相对起始时间的秒数。
- **命名规范**：新数据统一使用 `<robot>_<control_mode>_<backend>_<trajectory_id>.hdf5`，例如 `piper_joint_sim_000.hdf5` 或 `piper_pose_real_000.hdf5`。
...
```bash
# 关节空间记录（默认二值夹爪）
PYTHONPATH=. python scripts/sim/collect_data.py --mode joint

# 位姿空间记录（连续夹爪控制）
PYTHONPATH=. python scripts/sim/collect_data.py --mode pose
```
