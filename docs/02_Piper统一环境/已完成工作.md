...
本文档用于记录 `agent_infra/Piper_Env` 相对于 [开发计划.md](开发计划.md) 的当前完成进度。
...
- `PiperEnv` 已收敛为 teleop / 专家介入层：
  ...
  - 在 `step()` 中覆写实际执行动作
- 新增最终用户入口类：
  - [single_piper_env.py](../../agent_infra/Piper_Env/Env/single_piper_env.py)
  - [dual_piper_env.py](../../agent_infra/Piper_Env/Env/dual_piper_env.py)
...
- [piper_base_env.py](../../agent_infra/Piper_Env/Env/utils/piper_base_env.py)
- [single_piper_env.py](../../agent_infra/Piper_Env/Env/single_piper_env.py)
- [dual_piper_env.py](../../agent_infra/Piper_Env/Env/dual_piper_env.py)
...
- [piper_base_env.py](../../agent_infra/Piper_Env/Env/utils/piper_base_env.py)
- [piper_arm.py](../../agent_infra/Piper_Env/Env/utils/piper_arm.py)
...
- `PiperCameraWrapper` 追加：
  - `obs/rgb/...`
- 已补齐单臂 / 双臂配置入口：
  - [piper_config.yaml](../../agent_infra/Piper_Env/Config/piper_config.yaml)
  - [dual_piper_config.yaml](../../agent_infra/Piper_Env/Config/dual_piper_config.yaml)
...
- 已支持读取：
  ...
  - `crop_resolution`
- 已新增：
  - [orbbec_camera.py](../../agent_infra/Piper_Env/Camera/orbbec_camera.py)
  - `OrbbecCamera`
  - `OrbbecCameraGroup`
...
- [piper_camera_wrapper.py](../../agent_infra/Piper_Env/Env/piper_camera_wrapper.py)
- [realsense_camera.py](../../agent_infra/Piper_Env/Camera/realsense_camera.py)
- [orbbec_camera.py](../../agent_infra/Piper_Env/Camera/orbbec_camera.py)
...
- [recorder.py](../../agent_infra/Piper_Env/Record/recorder.py)
- [replay.py](../../agent_infra/Piper_Env/Record/replay.py)
- [postprocess.py](../../agent_infra/Piper_Env/Record/postprocess.py)
...
- 已新增：
  - [scripts/piper/single](../../scripts/piper/single)
  - [scripts/piper/dual](../../scripts/piper/dual)
- single / dual 下目前都已有：
  - 数据采集脚本
  - 轨迹回放脚本
  - H5 合并脚本
  - H5 -> LeRobot 转换脚本
  - LeRobot -> H5 转换脚本
- Piper 专用 CAN 脚本已迁移到 `scripts/piper/`
- `scripts/` 已完成脚本根目录收敛：
  - 所有运行脚本统一保留在 `scripts/` 的功能子目录。
  - Piper CAN、Orbbec、单臂/双臂采集、转换、回放和统一环境测试入口统一归入 `scripts/piper/`。
  - 旧 `agent_infra/Piper_Env/Script/`、`agent_infra/Realman_Env/Script/`、`piper_scripts/`、`agent_factory/script/`、`scripts/legacy/`、根目录重复 wrapper、旧 CAN 副本、无效 archive 片段和脚本缓存已删除。
- 已新增统一硬件 smoke test：
  - [test_piper_pipeline.py](../../scripts/piper/test_piper_pipeline.py)
- 已删除旧 Piper_Env Script 顶层脚本：
  - `collect_h5.sh`
  - `collect_lerobot.sh`
  - `replay.sh`
...
- 旧 ArmStudio 仿真采集脚本已改放到 `scripts/sim/collect_data.py`，不再通过根目录兼容壳调用。
...
- 已新增当前进度文档：
  - [已完成工作.md](已完成工作.md)
- 已新增使用与结构说明：
  - [Piper环境说明.md](Piper环境说明.md)
- 已新增根目录结构整理提案：
  - 历史 Piper_test 的 README/PLAN/DONE/协议草案已归档到 [legacy_piper_test](legacy_piper_test)
...
- Piper 专用 CAN 脚本已移动到 `scripts/piper/`。
...
- 采集命名已统一为 `<robot>_<control_mode>_<backend>_<trajectory_id>.hdf5`，例如 `piper_joint_sim_000.hdf5`；任务目录使用 `piper_single_joint_real_h5_task`、`piper_dual_joint_sim_h5_task` 这类结构，避免 real/sim、single/dual 混在一起。
...
- 结构整理提案第一批已执行：CAN 脚本移至 `scripts/piper/can/`，Orbbec 标定/SDK 探测脚本移至 `scripts/piper/camera/`，Orbbec SDK 日志归档到 `Log/hardware_debug/`。
...
- 新增 `scripts/piper/camera/check_orbbec_depth.py`，用于单独验证 Orbbec `v4l2/sdk/auto` 后端是否能返回 RGB/depth 帧。
...
- 新增 `scripts/piper/test_master_gripper_signal.py`，用于实机打印 master gripper 原始读数、过滤后读数与候选跳变计数。
...
- 针对 `can_sr` 实测 reset 可动但遥操连续跟随不响应的问题，新增 `joint_stream_command` 配置；`auto` 模式下 `can_sr` 自动从低延迟 `move_js` 回退到更稳的 `move_j`，双臂右从臂配置已显式设置为 `move_j`。
...
- 原 `Piper_test` 已完成独立性收口：`script/` 已对应到 `scripts/training/`，`trial/` 临时实验因用户已有外部备份不再保留，`pyorbbecsdk` 源码快照已迁入 `third_party/pyorbbecsdk/`，Piper Env 历史文档已归档到 `docs/02_Piper统一环境/legacy_piper_test/`。外层 `Piper_test/` 已删除，当前统一环境主链路不再依赖它。
- 历史真实采集数据 `Piper_test/agent_infra/Piper_Env/Record/data/` 约 2.5G，已按用户说明外部备份，本仓库不重复复制；后续使用旧数据时应整理到 `datasets/` 并转换为统一 H5 协议。
- `Arm_studio` 已明确作为项目根目录；外层 `3DGS/` 暂不移动，等进入 RoboSimGS/3DGS 阶段再融合。
- `scripts/piper/collect_unified.py` 已降级为统一配置 dry-run/协议检查工具，不再作为日常采集入口；真实采集走 `scripts/piper/single|dual`，仿真 H5 走 `scripts/piper/sim/collect_h5.sh` -> `scripts/sim/collect_data.py`。
- Piper 仿真 agent 已拆成 `piper_arm_right` 与 `piper_arm_left`；right/left 分别加载 `piper_description_with_camera_right.urdf` 与 `piper_description_with_camera_left.urdf`，腕部相机仍按 right hand 的正确方式挂在 `hand_cam`，外参写在各自 URDF 的 `hand_camera_joint`。
- 新增 `scripts/data_tools/convert_legacy_armstudio_h5.py`，可将旧 Arm_studio 顶层 `action + observation/...` HDF5 转为统一 `obs/... + action/... + meta/env_meta` raw H5；新输出命名遵循 `piper_joint_sim_000.hdf5` 规则。
- 新增依赖拆分文件：`requirements-base.txt`、`requirements-hardware.txt`、`requirements-sim.txt`、`requirements-train.txt`。
- 完成脚本目录彻底合并，并补充 `scripts/README.md` 与 `scripts/piper/README.md`；`collect_unified.py` 与 `test_unified_env.py` 的 `unified_real_dual.yaml --dry-config` 验证通过。
- `scripts/piper/can/can_quick_activate.sh` 激活四路 CAN 后会自动对 `can_sl`、`can_sr` 调用 `set_follower_mode()`，避免从臂被误设为主导臂后影响单臂/双臂遥操。
